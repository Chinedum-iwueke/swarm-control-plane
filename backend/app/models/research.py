...
class ResearchDailyCycleEvent(Base):
    __tablename__ = "research_daily_cycle_events"
    __table_args__ = (UniqueConstraint("cycle_id", "sequence"),)
    ...
    cycle_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("research_daily_cycles.id"),
        nullable=False,
        index=True,
    )
...
class ResearchMemoryExport(Base):
    __tablename__ = "research_memory_exports"

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    repository: Mapped[str] = mapped_column(String(150), nullable=False, index=True)
    repository_commit: Mapped[str] = mapped_column(String(64), nullable=False)
    database_digest: Mapped[str] = mapped_column(String(64), nullable=False)
    export: Mapped[dict] = mapped_column(JSONB, nullable=False)
    export_digest: Mapped[str] = mapped_column(String(64), nullable=False, unique=True)
    registered_by: Mapped[str] = mapped_column(String(150), nullable=False)
    registered_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
