# Alpha Loop Production-Parity Rehearsal

The autonomous alpha loop must not spend production approvals or campaign attempts
to discover runtime, routing, validation, or review defects. Before a native alpha
strategy engineer is activated, the installer runs a no-capital rehearsal inside
the same restricted systemd boundary.

The rehearsal uses a disposable Git repository and synthetic evidence. It walks the
...
Activation fails closed unless the rehearsal report says `success: true`, the
generic coder is ineligible, the dedicated engineer is eligible, the variant count
is eight, the window is 365 days, and capital/order authority is false. Production
